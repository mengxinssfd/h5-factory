<template>
  <div :class="['comp-content', component.active ? 'active' : '']" :style="getStyle">
    <div class="page-list">
      <div v-for="item in list" class="page-list-item" :style="getItemStyle('vertical-list-item')">
        ...
        <div class="page-list-item__bd">
          <div class="page-list-item__title" :style="getItemStyle('vertical-list-title_')">
            {{ item.title }}
          </div>
          <p class="page-list-item__desc" :style="getItemStyle('vertical-list-desc_')">
            {{ item.desc }}
          </p>
        </div>
      </div>
    ...
    getItemStyle(key) {
      ...
      this.component.others.config.forEach((item) => {
        ...
        if (isItem === 0) {
          const unit = item.unit || '';
          item.val &&
            ret.push(item.attr.substring(idx + 1, item.attr.length) + ':' + item.val + unit);
        }
      });
      return ret.join(';');
    },
